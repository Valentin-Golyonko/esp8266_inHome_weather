...
loadCSVAir(); // Download the CSV data, load Google Charts, parse the data, and draw the chart
...
function loadCSVAir() {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            google.charts.setOnLoadCallback(updateViewportAir);
        }
    };
...
function drawChartAir() {
	...
	navAirdiv.style.display = "compact";
...
function displayDateAir() { // Display the start and end date on the page
    var dateDiv = document.getElementById("dateAir");
...
document.getElementById("prevAir").onclick = function() {
...
document.getElementById("nextAir").onclick = function() {
...
document.getElementById("zoomoutAir").onclick = function() {
    zoomLevel += 1; // increment the zoom level (zoom out)
    if(zoomLevel > maxZoom) zoomLevel = maxZoom;
    else updateViewportAir();
}
document.getElementById("zoominAir").onclick = function() {
    zoomLevel -= 1; // decrement the zoom level (zoom in)
    if(zoomLevel < minZoom) zoomLevel = minZoom;
    else updateViewportAir();
}

document.getElementById("resetAir").onclick = function() {
...
document.getElementById("refreshAir").onclick = function() {
    viewportEndTime = new Date(); // the end time of the viewport is the current time
...
document.body.onresize = drawChartAir;

function updateViewportAir() {
