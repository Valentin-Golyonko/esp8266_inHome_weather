...
loadCSVPre(); // Download the CSV data, load Google Charts, parse the data, and draw the chart
...
function loadCSVPre() {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            google.charts.setOnLoadCallback(updateViewportPre);
        }
    };
...
function drawChartPre() {
	...
	navPrediv.style.display = "compact";

    var loadingdiv = document.getElementById("loading");
    loadingdiv.style.visibility = "hidden";
}

function displayDatePre() { // Display the start and end date on the page
    var dateDiv = document.getElementById("datePre");
...
document.getElementById("prevPre").onclick = function() {
...
document.getElementById("nextPre").onclick = function() {
...
document.getElementById("zoomoutPre").onclick = function() {
    zoomLevel += 1; // increment the zoom level (zoom out)
    if(zoomLevel > maxZoom) zoomLevel = maxZoom;
    else updateViewportPre();
}
document.getElementById("zoominPre").onclick = function() {
    zoomLevel -= 1; // decrement the zoom level (zoom in)
    if(zoomLevel < minZoom) zoomLevel = minZoom;
    else updateViewportPre();
}

document.getElementById("resetPre").onclick = function() {
...
document.getElementById("refreshPre").onclick = function() {
    viewportEndTime = new Date(); // the end time of the viewport is the current time
...
document.body.onresize = drawChartPre;

function updateViewportPre() {
